          ...
          <div className="card border-0 shadow-sm h-100 overflow-hidden transition-all hover-lift">
            <div className="card-body p-4">
              <div className="d-flex align-items-center mb-3">
                ...
                <div>
                  <h6 className="text-muted mb-0 small fw-bold text-uppercase tracking-wider">Total des absences</h6>
      ...
      <div className="card border-0 shadow-sm mb-5">
        ...
        <div className="card-body bg-light border-bottom">
          <div className="row g-4 align-items-end">
            ...
            <div className="col-lg-7">
              <label className="form-label fw-bold small text-muted text-uppercase">Période d'affichage</label>
              <div className="position-relative">
                ...
                {showCalendar && (
                  <div className="position-absolute end-0 mt-2 bg-white p-3 border rounded shadow-lg" style={{ minWidth: '350px', zIndex: 1050 }}>
        ...
        {dateColumns.length > 0 && filiere ? (
          filteredStagiaires.length > 0 ? (
              ...
              <table className="table table-bordered table-sm mb-0 align-middle professional-grid">
                <thead>
                  <tr className="bg-white">
                    <th className="ps-4 border-bottom-0 pb-3 pt-3 sticky-col" style={{ width: "240px", zIndex: 10 }}>
                      NOM DU STAGIAIRE
                    </th>
                    {dateColumns.map((d, index) => (
                      <th key={index} className="text-center py-2 border-bottom-0" style={{ minWidth: "60px", backgroundColor: '#f8f9fa' }}>
                        <div className="text-uppercase text-muted lh-1 mb-1" style={{ fontSize: '0.6rem' }}>
                          {d.toLocaleDateString('fr-FR', { weekday: 'short' })}
                        </div>
                        <div className="fw-bold text-dark">{d.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}</div>
                      </th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filteredStagiaires.map((stagiaire) => (
                    <tr key={stagiaire.id} className="hover-row">
                      ...
                      {dateColumns.map((d, dIdx) => {
                        ...
                        const totalHours = dayAbsences.reduce((sum, a) => sum + (a.heures || 2), 0);
                        const allJustified = dayAbsences.length > 0 && dayAbsences.every(a => a.justifie);

                        let cellClass = "";
                        let cellContent = null;

                        if (totalHours > 0) {
                          cellClass = allJustified ? "is-justified" : "is-absent";
                          cellContent = `${totalHours}h`;
                        } else {
                          cellContent = <span className="text-muted opacity-25">-</span>;
                        }

                        return (
                          <td key={dIdx} className={`text-center p-0 cell-slot ${cellClass}`} style={{ height: '48px' }}>
                            {cellContent}
                          </td>
                        );
                      })}
            ...
            <div className="text-center py-5 bg-white">
              <h5 className="text-muted">Aucun stagiaire trouvé pour cette filière</h5>
            </div>
          )
        ) : (
          <div className="text-center py-5 bg-white">
            <h5 className="text-muted"><i className="bi bi-info-circle me-2"></i>Sélectionnez une filière pour afficher l'historique</h5>
          </div>
        )}
      </div>
